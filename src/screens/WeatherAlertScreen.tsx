...
import { Home, Sun, CloudSun, Cloud } from 'lucide-react-native';
            ...
            <SafeAreaView className="flex-1" style={{ paddingTop: Platform.OS === 'android' ? StatusBar.currentHeight : 0 }}>
                ...
                >
                    ...
                    <View className="flex-row justify-between items-start px-6" style={{ marginTop: hp(3) }}>
                        <View>
                            <Text className="font-bold text-[#1C1C1E] leading-tight" style={{ fontSize: wp(8) }}>Weather Based,</Text>
                            <Text className="font-normal text-[#1C1C1E] leading-tight" style={{ fontSize: wp(8) }}>Alerts</Text>
                        </View>
                    ...
                    {/* --- Koala Image Section --- */}
                    <View className="items-center justify-center" style={{ marginTop: hp(2), marginBottom: hp(2) }}>
                        <Image
                            source={require('../assets/kolaweather.png')}
                            style={{ 
                                width: wp(65), 
                                height: wp(65), 
                            }}
                    ...
                    <View className="px-6 mb-6">
                        <Text className="font-bold text-[#000] mb-4" style={{ fontSize: wp(5) }}>Weather Forecast</Text>
                        
                        {/* Today Card */}
                        <ForecastCard
                            icon={<Sun color="#F59E0B" size={28} fill="#F59E0B" />}
                            title="Today"
                            subtitle="Sunny"
                            temp="28°C"
                        ...
                        {/* Tomorrow Card */}
                        <ForecastCard
                            icon={<CloudSun color="#056B87" size={28} />}
                            title="Tomorrow"
                        ...
                        {/* Next Day Card */}
                        <ForecastCard
                            icon={<Cloud color="#056B87" size={28} />}
                            title="Next Day"
                            subtitle="Cloudy"
                            temp="25°C"
                            bgColor="rgba(240, 248, 255, 0.6)"
                    ...
                    <View className="px-6 mb-4">
                        <Text className="font-bold text-[#000] mb-2" style={{ fontSize: wp(5) }}>Smart Recommendations</Text>
                        <Text className="text-[#666] leading-5" style={{ fontSize: wp(3.5) }}>
                ...
                >
                    ...
                    {/* Active Icon (User) */}
                    <TouchableOpacity><UserIcon color="#056B87" filled={true} /></TouchableOpacity>
...
const ForecastCard = ({ icon, title, subtitle, temp, bgColor }: any) => (
    <View 
        className="flex-row items-center justify-between p-4 rounded-[16px] mb-3"
    ...
    >
        <View className="flex-row items-center">
            ...
            <View>
                <Text className="font-bold text-[#0D1B2A]" style={{ fontSize: wp(4) }}>{title}</Text>
                <Text className="text-[#666]" style={{ fontSize: wp(3.2) }}>{subtitle}</Text>
            </View>
        </View>
        <Text className="font-bold text-[#0D1B2A]" style={{ fontSize: wp(4.5) }}>{temp}</Text>
    </View>
);
