            ...
            <SafeAreaView className="flex-1" style={{ paddingTop: Platform.OS === 'android' ? StatusBar.currentHeight : 0 }}>
                ...
                >
                    ...
                    <View className="flex-row justify-between items-start px-6" style={{ marginTop: hp(3) }}>
                        <View>
                            <Text className="font-bold text-[#1C1C1E] leading-tight" style={{ fontSize: wp(8) }}>AI based,</Text>
                            <Text className="font-normal text-[#1C1C1E] leading-tight" style={{ fontSize: wp(8) }}>Smart Suggestions</Text>
                        </View>
                    ...
                    {/* --- Top Koala Banner --- */}
                    <View className="mx-6 relative mt-6 mb-8">
                        {/* Background Card */}
                        <View 
                            className="bg-[#EFEBE9] rounded-[24px] p-5 flex-row items-center justify-end"
                            style={{ 
                                height: hp(15), 
                                marginLeft: wp(10), // To make space for Koala on left
                                backgroundColor: 'rgba(239, 235, 233, 0.6)' // Semi-transparent
                            }}
                        >
                            <Text 
                                className="text-[#4A3B32] font-medium text-right leading-5" 
                                style={{ width: '85%', fontSize: wp(3.8) }}
                            >
                                AI Tracks your power usages and solar use to suggest smarter, cost-saving choices
                            </Text>
                        </View>
                        
                        {/* Koala Image (Overlapping Left) */}
                        <Image
                            source={require('../assets/thinkola.png')}
                            className="absolute"
                            style={{ 
                                left: -10, 
                                bottom: 0, 
                    ...
                    <View className="px-6 mb-4">
                        <Text className="font-bold text-[#424242] mb-5" style={{ fontSize: wp(5) }}>Schedule Appliance Usage</Text>
                        
                        {/* Suggestion Cards */}
                        <SuggestionCard
                            text="Use Washing machine during peak solar hours to save more energy"
                            image={require('../assets/img1.jpg')}
                        />
                        
                        <SuggestionCard
                            text="To save more electricity, operate the water motor during times when solar energy is received at its maximum."
                            image={require('../assets/img2.jpg')}
                        />

                        <SuggestionCard
                            text="To reduce electricity cost, do the ironing during the daytime when sunlight is good."
...
const SuggestionCard = ({ text, image }: any) => (
    <View className="bg-[#EFF6F9] rounded-[20px] p-4 flex-row items-center justify-between mb-4 shadow-sm h-[110px] overflow-hidden">
        {/* Text Section */}
        <View style={{ width: '60%' }}>
            <Text className="text-[#666] leading-5 font-medium" style={{ fontSize: wp(3.2) }}>
                {text}
            </Text>
        </View>
